...
def fetch_recent_reels(username: str, max_retries: int = 2) -> list:
    """Récupère les derniers Reels via RapidAPI avec retry et timeout généreux."""
    ...
    for attempt in range(max_retries):
        try:
            r = requests.get(
                ...
                headers=headers,
                timeout=60,  # timeout généreux pour comptes lents
            )
            if not r.ok:
                log.warning("RapidAPI %s -> %s (try %d)",
                            username, r.status_code, attempt + 1)
            ...
            log.warning("Fetch Insta %s timeout (try %d/%d)",
